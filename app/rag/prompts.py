...
# 5. 악성 민원 필터링 및 질문 정제용 프롬프트
filter_and_sanitize_prompt = ChatPromptTemplate.from_messages(
    [
        ("system",
        """
        **역할:** 당신은 '민원 정제 AI'입니다. 감정적이고 비공식적인 민원인의 원본 질문을 분석하여, 담당 공무원이 처리하기 용이하도록 중립적이고 사실 기반의 핵심 요지로 정제하는 임무를 맡았습니다.
        **지시사항:**
        1.  아래 '원본 질문'에서 욕설, 비속어, 인신공격 등 불필요한 공격적 표현을 모두 제거하십시오.
        2.  감정적인 호소나 불평을 객관적인 사실 관계로 변환하십시오. (예: "너무 시끄러워 미치겠어요! 빨리 처리해요" -> "지속적인 소음으로 불편을 겪고 있음")
        3.  여러 문장으로 흩어져 있는 내용을 육하원칙에 따라 명확한 하나의 문단으로 요약하십시오.
        4.  최종 결과물은 오직 '정제된 민원 내용'만 포함해야 합니다.
        """),
        ("human", "[원본 질문]\n{question}")
    ]
)
...
# 6. 최종 보고서 생성 프롬프트(추후 JSON 형태로 변환 예정)
create_final_report_prompt = ChatPromptTemplate.from_messages(
    [
        ("system",
        """
        **역할:** 당신은 민원 처리 결과를 최종적으로 조립하는 AI 에이전트입니다. 당신의 결과물은 민원인에게 보여줄 '공식 답변'과, 담당자가 내부적으로 참고할 '정제된 민원 정보'를 모두 포함해야 합니다.

        **목표:** 주어진 모든 정보를 바탕으로, 아래의 최종 보고서 템플릿을 완벽하게 완성합니다.

        **지시사항:**
        1.  [민원 검토 결과 안내] 섹션에는 '답변 초안'을 바탕으로 민원인에게 제공할 공식 답변을 작성하십시오.
        2.  [담당자 참고 정보] 섹션에는 '정제된 민원 요약'과 '민원인 원본 질문'을 그대로 기재하십시오.
        """),
        ("human",
        "[사용자 원본 질문]\n{question}\n\n"
        "[답변 초안]\n{answer}\n\n"
